...
crop_save = "/home/ubuntu/tess_train_data_prep/crops/tamil/word_crop_save/"
...
def filter_low_conf_word(words_dict1,words_dict2):
    final_dict={"text":[],"conf":[]}; final_text = ""
    top_dict= None
    flag= 1
    if len(words_dict1['conf'])>=len(words_dict2['conf']):
        top_dict = words_dict1
    else:
        top_dict = words_dict2
        flag=2
    for idx,conf in enumerate(top_dict['conf']):
        text = top_dict['text'][idx]
        text2=None; text1 = text
        if flag==1:
            match = difflib.get_close_matches(text, words_dict2['text'])
            if len(match)>0:
                idx2 = words_dict2['text'].index(match[0])
                conf2 = words_dict2['conf'][idx2]
                text1 = words_dict2['text'][idx2]
            else:
                conf2=conf
                text2=text
        else:
            match = difflib.get_close_matches(text, words_dict1['text'])
            if len(match)>0:
                idx2 = words_dict1['text'].index(match[0])
                conf2 = words_dict1['conf'][idx2]
                text1 = words_dict1['text'][idx2]
            else:
                conf2=conf
                text2=text
        if conf>=conf2:
            if text2==None:
                ...
                final_dict['conf'].append(conf)
                final_text +=text
            else:
                final_dict['text'].append(text2)
                final_dict['conf'].append(conf)
                final_text +=text2
        else:
            if text2==None:
                final_dict['text'].append(text1)
                final_dict['conf'].append(conf2)
                final_text +=text1
            else:
                final_dict['text'].append(text2)
                final_dict['conf'].append(conf2)
                final_text +=text2
    return final_text,final_dict

    
def process_dfs_updated(temp_df,language,psm_val,image):

    temp_df = temp_df[temp_df.text.notnull()]
    org_text,org_dict  = process_dfs(temp_df)
    text = "";  temp_dict1 = {"text":[],"conf":[]}
    if len(temp_df)>0:
        for index, row in temp_df.iterrows():
            org_conf = row["conf"]; org_text = row['text']
            if row["conf"]<=config.conf and config.double_ocr_level=='WORD':
                try:
                    crop_image = image[ int(row["top"])-7:int(row["top"]+row["height"]+7), int(row["left"]-10):int(row["left"]+row["width"]+10)]
                    save_path  =  crop_save+str(uuid.uuid4()) + '.jpg'
                    cv2.imwrite(save_path,crop_image)
                    df2 = pytesseract.image_to_data(crop_image,config='--psm '+str(8), lang=tess_lang2,output_type=Output.DATAFRAME)
                    new_text,tmp_dict = process_dfs(df2)
                    if len(tmp_dict['conf'])>1:
                        new_conf =max(tmp_dict['conf'])
                    elif len(tmp_dict['conf'])==1:
                        new_conf =tmp_dict['conf'][0]
                    else:
                        new_conf = org_conf
                    ...
                    pass
            if row["conf"]<=config.conf and config.double_ocr_level=='LINE':
                df2 = pytesseract.image_to_data(image,config='--psm '+str(psm_val), lang=tess_lang2,output_type=Output.DATAFRAME)
                new_text,tmp_dict = process_dfs(df2)
                new_text,tmp_dict = filter_low_conf_word(org_dict,tmp_dict)
                return new_text,tmp_dict
            temp_dict1["text"].append(org_text); temp_dict1["conf"].append(org_conf)
            text = text +" "+ str(org_text)
    if  config.double_ocr_level=='LINE':
        return process_dfs(temp_df)
    return text,temp_dict1
...
#	if psm_val==8:
    ...
    if tess_ocr=="True":
        if abs(coord[1,1]-coord[2,1])>mode_height:
            dfs = pytesseract.image_to_data(crop_image,config='--psm 6', lang="Tamil",output_type=Output.DATAFRAME)
            ...
            if psm_val==8:
                dfs = pytesseract.image_to_data(crop_image,config='--psm '+str(psm_val), lang="Tamil",output_type=Output.DATAFRAME)
            else:
...
def tesseract(json_files):
    ...
    for json_file in json_files:
        ...
        final_df.to_csv(save_base_path+"/"+file_name+"_"+str(tess_lang)+"_"+str(tess_lang2)+"_"+str(config.double_ocr_level)+"_"+str(config.conf)+"_high_conf_word"+'.csv')
