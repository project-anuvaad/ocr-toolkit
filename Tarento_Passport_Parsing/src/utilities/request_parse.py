...
class File:
    ...
    @log_error
    def set_sr(self):
        if "superresolution" in self.file["config"].keys():
            config.SUPER_RESOLUTION = self.file["config"]["superresolution"]
            return self.file["config"]["superresolution"]
        else:
            config.SUPER_RESOLUTION = False
            return False
    ...
    def set_image_source(self, im_index):
        if (
            ...
            "local_path" in self.file["image"][im_index].keys()
            and self.file["image"][im_index]["local_path"] is not None
        ):
            self.im_source = "local_path"
        else:
            self.im_source = None
    ...
    def get_image(self, im_index):
        ...
        elif self.im_source is "local_path":
            img_path=config.LOCAL_IMG_PATH+self.file["image"][im_index][self.im_source]
