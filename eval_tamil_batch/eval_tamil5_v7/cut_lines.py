...
def to_df(data_csv,folder,folder_name,prefix):     
    data_df = pd.DataFrame(data_csv,columns=['path','boundingBox','key','score','g_conf','tess_text','ground_text','id'])    
...
def crop_image_with_df(image,page_path, df ,folder_name,job_id,page_index,prefix):
    ...
    for line_index in range(len(df)):
        try:
            ...
            identifier= df['identifier'].iloc[line_index]
            ...
            data_csv.append([page_path, bbox, key,score,g_conf ,t_text, ground_text,identifier])
...
def get_error_crops(eval_csv_path):
    ...
    for page_index in range(len(eval_df)):
        ...
        if 'g_conf'  not in page_df.keys():
            page_df['g_conf']=0.90
        if 'score' not in page_df.keys():
            page_df['score']=0
