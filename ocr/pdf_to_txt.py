...
class TokenizePdf:
    ...
    def upload(self):
        ...
        print("file upload successful",files)
        try:
    ...
    def get_language(self):
        if self.file_id is not None:
            ...
            if len(self.lang) is not 2:
                logging.error('Invalid laguage code {} for file {}'.format(
                    self.lang, self.file_name))
                self.lang = None
        else:
            self.lang = None

    def ocr_and_tokenize(self):
        print("ocr_and_tokenize started")
