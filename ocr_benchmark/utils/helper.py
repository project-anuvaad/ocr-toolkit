...
def tess_eval(input):
    ...
    ocr_text, ocr_conf = get_tess_text(image_crop, lang, mode_height,crop_name)
...
#    return [gt_text, ocr_text, score, gt_len, mismatch_count, str(config.EXP_NAME) + '_' + lang, coord, image_name, crop_name, ocr_conf]
    ...
    append_line( [gt_text, ocr_text, score, gt_len, mismatch_count, str(config.EXP_NAME) + '_' + lang, coord, image_name, crop_name, ocr_conf])
...
def add_lines_to_tess_queue(gt_source, queue, lang):
    ...
    for csv_path in csv_paths:
        ...
        if len(page_data) > 0 and image is not None:
            ...
            for index,coord in enumerate(coords):
                ...
                class_name=page_data['class'].iloc[index]
                image_crop = crop_region(coord,image,class_name,crop_name)
                if image_crop is not None and image_crop.shape[1] >3 and image_crop.shape[0] > 3:
                    line_meta_data = [lang, image_crop, page_data['groundTruth'].iloc[index],coord,mode_height,image_name ,crop_name]
                    tess_eval(line_meta_data)
                    #queue.put(line_meta_data) 
        else:
            print("issues in page data", page_data,image)

def add_org_lines_to_tess_queue(gt_source, queue, lang):
...
#                if row == None:
 #                   continue
                crop_name=row['images']
                class_name=row['class']
                image_crop = cv2.imread(crop_name)
                if image_crop is not None and image_crop.shape[1] >3 and image_crop.shape[0] > 3:
                    line_meta_data = [lang, image_crop, row['groundTruth'],None,image_crop.shape[0],crop_name ,crop_name]
                    #tess_eval(line_meta_data)
                    queue.put(line_meta_data) 
        else:
            print("issues in page data", page_data)
