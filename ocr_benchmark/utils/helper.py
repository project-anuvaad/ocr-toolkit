...
def init_file():
    if not os.path.exists(config.OUTPUT_FILE):
        columns = ['gt_text', 'tess_text', 'score', 'gt_lenght','mismatch_count' , 'exp', 'coord' ,'image_name', 'crop_name', 'ocr_conf']
        text = ','.join(columns) + '\n'
...
def tess_eval(input):
    ...
    ocr_text,ocr_conf  = post_process_ocr_text(image_crop,ocr_text,ocr_conf,mode_height)
...
def add_lines_to_tess_queue(gt_source,queue,lang):
    ...
    for csv_path in csv_paths :
        ...
        image_path = os.path.join(config.IMAGE_DIR,'{}.{}'.format(image_name,config.IMAGE_FORMAT))
        #print(image_path)
