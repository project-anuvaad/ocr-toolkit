...
def scale_coords(page_data,image_shape,page_resolution):
    ...
    for coord in coords:
        ...
        for point in coord['boundingBox']['vertices']:
            region['boundingBox']['vertices'].append({ 'x' : int(point['x'] * x_ratio), 'y': int(point['y'] * y_ratio) })
...
def crop_region(coord,image,crop_name):
    try:
        if validate_region(coord):
            ...
            config.logging.error('Error in region region {}  due to invalid coordinates'.format(coord))
            return None
