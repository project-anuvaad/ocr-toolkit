...
def cvt_coords_to_array(obj):
    ...
    return np.array([(float(pt["x"]), float(pt["y"])) for pt in obj.find_all("Point")])
...
def _create_category(schema=0):
    ...
    if schema == 0:
        ...
        categories = [{"supercategory": "layout", "id": 0, "name": "Line"}]
        ...
        find_categories = lambda name: [
            val["id"] for val in categories if val["name"] == name
        ][0]

        conversion = {
            "LineRegion": find_categories("LineRegion")
...
parser.add_argument(
    "--prima_datapath",
    type=str,
    default="/home/ubuntu/layout-model-training/data/prima",
    help="the path to the prima data folders",
)
parser.add_argument(
    "--anno_savepath",
    type=str,
    default="/home/ubuntu/layout-model-training/data/prima/annotations.json",
    help="the path to save the new annotations",
)
