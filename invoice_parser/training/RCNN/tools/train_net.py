...
class Trainer(DefaultTrainer):
    ...
    def eval_and_save(cls, cfg, model):
        #        print("modelllllllllllllllllllllllllll",model)
        evaluators = [
            cls.build_evaluator(
...
def startTensorboard(logdir):
    ...
    os.system("tensorboard --logdir={} --port=16007".format(logdir))
...
def main(args):
    ...
    resume = True  # config.RESUME
    if args.eval_only:
        model = Trainer.build_model(cfg)
        # print("modedddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddl",model)
    ...
    mod = trainer.model
    mod.to("cuda")
    #    mod.roi_heads.mask_head.predictor = torch.nn.Conv2d(256, 6,kernel_size=(1, 1), stride=(1, 1))
    #    in_features = mod.roi_heads.box_predictor.cls_score.in_features
    #    mod.roi_heads.mask_head.predictor =GeneralizedRCNN(in_features,
    #                                                       256,
    #                                                      6)

    #   newmodel = torch.nn.Sequential(*(list(mod.children())[:-1]))
    trainer.register_hooks([hooks.EvalHook(0, lambda: trainer.eval_and_save(cfg, mod))])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--eval_only", default="True", help="evaluation purpose with pretrained net"
    )
    #    parser.add_argument("--resume",                default="True", help="start training from the pause")
    ...
    register_coco_instances(
        f"{dataset_name}-train", {}, args.json_annotation_train, args.image_path_train
    )

    register_coco_instances(
        f"{dataset_name}-val", {}, args.json_annotation_val, args.image_path_val
    )

    Process(
        target=startTensorboard,
        args=(
            "/home/ubuntu/layout-model-training/outputs/prima/mask_rcnn_R_50_FPN_3x/",
        ),
    ).start()
