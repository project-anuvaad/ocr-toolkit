...
class RemoveOverlap:
    ...
    def polygon_overlap(self, reg1, reg2, org_reg1, org_reg2):
        ...
        if area1 > area2:
            if (
                keys.get_left(org_reg1) < keys.get_left(org_reg2)
                and keys.get_right(org_reg1) > keys.get_right(org_reg2)
                and keys.get_top(org_reg1) < keys.get_top(org_reg2)
                and keys.get_bottom(org_reg1) > keys.get_bottom(org_reg2)
            ):
                return True
        ...
        if area1 < area2:
            if (
                keys.get_left(org_reg2) < keys.get_left(org_reg1)
                and keys.get_right(org_reg2) > keys.get_right(org_reg1)
                and keys.get_top(org_reg2) < keys.get_top(org_reg1)
                and keys.get_bottom(org_reg2) > keys.get_bottom(org_reg1)
            ):
                return True
        ...
        if reg1.contains(reg2) or reg2.contains(reg1):
            return True
        if (
            keys.get_top(org_reg2) < keys.get_top(org_reg1)
            and keys.get_bottom(org_reg2) > keys.get_bottom(org_reg1)
        ) and (
            keys.get_top(org_reg2) > keys.get_top(org_reg1)
            and keys.get_bottom(org_reg2) < keys.get_bottom(org_reg1)
    ...
    def is_connected(self, region1, region2):
        ...
        if (
            ...
            or (
                area
                and abs(keys.get_top(region1) - keys.get_top(region2))
                < keys.get_height(region2) * 0.2
            )
            or (area and total_intsc_area > max(area1, area2) * 0.3)
        ):
            return True
        else:
            return False
